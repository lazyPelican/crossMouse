...
==================================
Works on BOTH Wayland and X11 by using Linux uinput (kernel-level input).

Usage:
    python3 client_gui.py
    # or after building:
    ./dist/MouseShareClient

First-time setup (run once):
    sudo chmod 666 /dev/uinput
    # (or use the setup button in the app)
"""
...
# This bypasses Wayland restrictions by creating virtual devices at the
# kernel level (/dev/uinput).  Works on both Wayland and X11.
...
# --- key mappings: pynput names / chars / VK codes  →  evdev keycodes ---
...
# Characters → evdev keycodes  (maps both shifted and unshifted to the base key)
_CHAR: dict[str, int] = {" ": ecodes.KEY_SPACE, "\t": ecodes.KEY_TAB,
...
# Windows virtual-key codes → evdev keycodes
_VK: dict[int, int] = {0x20: ecodes.KEY_SPACE}
for _i in range(26):
...
def _deser(k):
    """Deserialize a key tuple from the server into an evdev keycode."""
...
class Injector:
    """Creates virtual mouse + keyboard via /dev/uinput."""
    ...
    def __init__(self, screen_w=1920, screen_h=1080):
        ...
        kb_cap = {
            ecodes.EV_KEY: list(range(1, 272)),   # KEY_ESC..KEY_MAX typical range
        }
        self._m = UInput(mouse_cap, name="mouseshare-mouse")
        self._k = UInput(kb_cap,    name="mouseshare-kb")

    def move_to(self, x, y):
        """Move cursor to absolute (x, y) by overshooting to (0,0) then moving to target."""
        # Slam cursor to top-left corner
        self._m.write(ecodes.EV_REL, ecodes.REL_X, -(self._sw + 500))
        self._m.write(ecodes.EV_REL, ecodes.REL_Y, -(self._sh + 500))
        self._m.syn()
        time.sleep(0.03)
        # Now move to desired absolute position
        if x > 0 or y > 0:
...
def _can_uinput():
    """Check write access to /dev/uinput."""
    return (
...
def _screen_size():
    """Best-effort LOGICAL screen resolution (what the compositor uses for the cursor).

    On Wayland with scaling the native mode (e.g. 3840x2160) differs from
    the logical viewport (e.g. 1920x1080).  We need the logical one so our
    virtual-cursor tracking matches the real cursor's coordinate space.
    """
    # Method 1: xrandr OUTPUT LINE  (e.g. "eDP-1 connected primary 1920x1080+0+0")
    #   The WxH+X+Y on the output line is the logical viewport size.
    #   Prefer the 'primary' output; fall back to the first connected one.
    try:
    ...
    # Method 2: tkinter (returns logical size on most setups)
    try:
        r = tk.Tk(); r.withdraw()
...
class ClientApp:
    def __init__(self):
        ...
        self._held: list[int] = []      # evdev keycodes currently pressed
        self._edge_cd = 0.0
        ...
        # Cross-check with tkinter's own screen dimensions (logical on Wayland).
        # Take the smaller value — it's more likely the real cursor space.
        try:
            ...
            pass
        # Update the screen label with the corrected dimensions
        try:
        ...
        self._tick()
        self._sync_cursor_pos()               # periodic real-cursor edge check
    ...
    def _build_ui(self):
        ...
        # uinput permission banner (shown only if needed)
    ...
    def _copy_fix_cmd(self):
        ...
        self._ui("log", "Copied command to clipboard")

    def _tick(self):
        while True:
            try: kind, val = self._q.get_nowait()
            except queue.Empty: break
            if kind == "log":   self._append_log(val)
            elif kind == "conn":
    ...
    # ── periodic real-cursor edge check (runs on main / GUI thread) ─────
    def _sync_cursor_pos(self):
        """Query the REAL cursor position via tkinter and trigger edge
        switch if it's at the return edge.  This catches cases where the
        virtual vx/vy tracker drifts from reality (e.g. Wayland scaling,
        compositor clamping mismatch).  Runs every 50 ms on the main thread.
        """
    ...
    def _start_connect(self, host=None, port=None, trusted_name=None, silent=False):
        ...
        if _EVDEV_IMPORT_ERROR is not None:
            if not silent:
                messagebox.showerror(
                    "Missing dependency",
                    "The Linux 'evdev' package is required for mouse and keyboard injection.\n\n"
                    "Install it with:\n"
                    "  python3 -m pip install evdev\n\n"
                    "Then start Mouse Share Client again.")
            return
        ...
        if not self.injector:
            try:
                ...
                self._ui("log", "Virtual mouse + keyboard created")
        ...
        self._ui("log", f"Connecting to {host}:{port} …")
    ...
    def _conn_loop(self, host, port, trusted_name=None):
        while not self._stop.is_set():
            ...
            try:
                ...
                self._ui("log", f"Connected to {host}")
                ...
                self._ui("log", "Connection refused, retrying …")
            except OSError as e:
                self._ui("log", f"Connection lost ({e})")
    ...
    def _handle(self, msg):
        ...
        if t == "sw":
            ...
            self._edge_cd = time.time() + 1.5   # grace period before edge-detect

            yr = msg.get("yr", 0.5); xr = msg.get("xr", 0.5)
            y = max(0, min(int(yr * self.SH), self.SH - 1))
            x = max(0, min(int(xr * self.SW), self.SW - 1))

            # Place cursor at the entry edge (e.g. left edge if Ubuntu is right of Windows)
            ...
            # ACTUALLY move the cursor to entry position
            self.injector.move_to(ex, ey)
            self.vx, self.vy = ex, ey

            self._ui("mode", "recv")
            self._ui("log", f">> Receiving input (cursor at {ex},{ey})")
